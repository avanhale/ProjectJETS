         ...
         bool isCluster = false)
      {
         ...
         for (int i = 0; i < src.Count; ++i)
         {
            try
            {
               ...
               {
                  for (int mip = 0; mip < traxDiffuseHeightTex.mipmapCount; ++mip)
                  {
               ...
               { 
                  for (int mip = 0; mip < traxNormalSAOTex.mipmapCount; ++mip)
                  {
               ...
               if (splatTex != null)
               {
                  for (int mip = 0; mip < splatTex.mipmapCount; ++mip)
                  {
                     Graphics.CopyTexture (splatTex, 0, mip, splatArray, i, mip);
                  }
               }
               ...
               {
                  for (int mip = 0; mip < smoothAOTex.mipmapCount; ++mip)
                  {
               ...
               if (antiTileTex != null)
               {
                  for (int mip = 0; mip < antiTileTex.mipmapCount; ++mip)
                  {
               ...
               if (specularTex != null)
               {
                  for (int mip = 0; mip < specularTex.mipmapCount; ++mip)
                  {
         ...
         string splatPath = GetSplatPath (cfg, ext);
         ...
         if (!cfg.IsDecalSplat ())
         {
            ...
            if (existing != null)
            {
               normalSAOArray.name = existing.name;
         ...
         if (cfg.traxArray && !isCluster)
         {
            ...
            if (existing != null)
            {
               traxDiffuseArray.name = existing.name;
         ...
         if (cfg.traxArray && !isCluster)
         {
            ...
            if (existing != null)
            {
               traxNormalSAOArray.name = existing.name;
         ...
         if (cfg.packingMode != TextureArrayConfig.PackingMode.Fastest && smoothAOArray != null)
         {
            ...
            if (existing != null)
            {
               smoothAOArray.name = existing.name;
         ...
         if (cfg.antiTileArray && antiTileArray != null)
         {
            ...
            if (existing != null)
            {
               antiTileArray.name = existing.name;
         ...
         {
            var existing = AssetDatabase.LoadAssetAtPath<Texture2DArray> (splatPath);
            if (existing != null)
            {
               splatArray.name = existing.name;

               EditorUtility.CopySerialized (splatArray, existing);
            }
            else
            {
               AssetDatabase.CreateAsset (splatArray, splatPath);
            }
         }
         ...
         if (cfg.pbrWorkflow == TextureArrayConfig.PBRWorkflow.Specular && specularArray != null)
         {
            ...
            if (existing != null)
            {
               specularArray.name = existing.name;
